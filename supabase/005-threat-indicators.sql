...
CREATE TABLE IF NOT EXISTS threat_indicators (
  ...
  indicator_type text NOT NULL CHECK (indicator_type IN (''message'', ''link'', ''screenshot'')),
...
CREATE POLICY IF NOT EXISTS "threat_indicators_anon_read"
  ON threat_indicators FOR SELECT USING (true);

-- Only service role can write/update (anon inserts are rejected)
CREATE POLICY IF NOT EXISTS "threat_indicators_service_write"
